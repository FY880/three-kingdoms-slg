...
1. **调数值**：直接改 `data/*.csv` 里的系数；`rules.js` 顶部内置数据与 CSV 同步，改完把 CSV 用 `csv-parse` 读入即可热替换（Demo 为离线内置，未接 fetch）。
...
- 逻辑层（Node）：环境结算、水攻（雨季低洼 3 回合灭敌）、庙算、战斗（平原/山地/伏击）均正常。
- Demo（DOM 桩冒烟测试）：地图生成、渲染、庙算、战报流程无运行期异常。
